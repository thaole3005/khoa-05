...
export default function BtTongHop1(props) {
  //loại bỏ các phần tử có cùng maDanhMuc (chỉ giữ lại 1 nếu có các phàn tử trùng nhau)
  const mangDanhMuc = _.unionBy(data, "maDanhMuc");
  ...
  return (
    <div>
        <Popup openModal={openModal} setOpenModal ={setOpenModal} foodItem={foodItem}/>
      <div className="grid grid-cols-6">
          ...
          <div className="divide-y divide-coolGray-300">
            <ul className="pt-2 pb-4 space-y-1 text-sm">
              {mangDanhMuc.map((item, index) => {
                  ...
                  const classNameActive = ['flex', 'items-center', 'p-2', 'space-x-3', 'rounded-md', maDanhMuc === item.maDanhMuc ? 'bg-black text-white' : ''];

                  const classNameActiveIcon = maDanhMuc === item.maDanhMuc ? 'bg-white' : '';
                  ...
                  >
                    ...
                    >
                      <svg
                        ...
                        viewBox="0 0 512 512"
                        className={`w-5 h-5 fillCurrent text-coolGray-600 ${classNameActiveIcon}`}
                      >
        ...
        <div className="col-span-5">
          <h3 className="text-center text-purple-500 text-6xl mt-3">Menu</h3>
                      ...
                      <div key={index} class="p-4 lg:w-1/3">
                        <div class="h-full bg-gray-100 bg-opacity-75 px-8 pt-16 pb-12 rounded-lg overflow-hidden text-center relative">
                          ...
                          <div class="leading-relaxed mb-3">
                            <img src={food.hinhAnh} alt={food.tenMonAn} />
                          </div>
                          <button class="text-indigo-500 inline-flex items-center" onClick = {() => {
                              setOpenModal(true);
                              setFoodItem(food)
                          }}>
                            Xem Chi Tiết
                          ...
                          <div class="leading-none flex justify-end absolute bottom-0 left-0 w-full py-4">
                            <span class="text-gray-400 mr-3 inline-flex items-center leading-none pr-3 text-2xl">
                              Gía {food.gia}
                            </span>
                          </div>
                        </div>
